...
class GrayscaleBlock(SampleBase):
    ...
    def run(self):
        ...
        x_step = max(1, (width+1) / sub_blocks)
        y_step = max(1, (height+1) / sub_blocks)
        count = 0
        ...
        while True:
            for y in range(0, height):
                for x in range(0, width):
                    ...
                    if count % 4 == 0 :
                        self.matrix.SetPixel(x, y, x, x, x)
                        #self.matrix.SetPixel(y, c, c, c, x)
                    elif count % 4 == 1 :
                        #self.matrix.SetPixel(x, y, c, 0, 0)
                        self.matrix.SetPixel(x, y, c, c, c)
                    elif count % 4 == 2:
                        #self.matrix.SetPixel(x, y, 0, c, 0)
                        self.matrix.SetPixel(x, y, y, y, y)
                    elif count % 4 == 3:
